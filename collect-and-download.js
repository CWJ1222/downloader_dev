...
function loadConfig() {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        console.error('   config.example.json을 복사하여 config.json을 만들고 강의 URL을 입력하세요.');
        process.exit(1);
    }
...
// 설정
const COURSE_URL = config.courseUrl;
const OUTPUT_FILE = './video-urls.json';
const OUTPUT_DIR = config.outputDir || './videos';
...
function downloadVideoOnce(clip, outputPath) {
    return new Promise((resolve) => {
        const ffmpeg = spawn('ffmpeg', [
            ...
            '-y',
            outputPath
        ], { stdio: ['ignore', 'ignore', 'pipe'] });  // stdout도 ignore
...
async function main() {
    ...
    await page.goto('https://kdt.fastcampus.co.kr/account/sign-in', { waitUntil: 'networkidle', timeout: 60000 });
    ...
    for (let partIdx = 0; partIdx < partToggles.length; partIdx++) {
        ...
        // Part 토글 다시 가져오기
        const currentPartToggles = await page.locator('.classroom-sidebar-clip__chapter__title').all();
    ...
    while (downloadQueue.length > 0 || activeDownloads > 0) {
        console.log(`  대기: ${downloadQueue.length}, 진행: ${activeDownloads}, 완료: ${downloadedCount}`);
        await new Promise(r => setTimeout(r, 2000));
    }
    ...
    console.log(`다운로드: ${downloadedCount}개 성공, ${downloadFailedCount}개 실패`);
    console.log(`${'='.repeat(60)}\n`);
    ...
    if (failedDownloads.length > 0) {
        ...
        fs.writeFileSync(failedFile, JSON.stringify(failedDownloads.map(f => f.clip), null, 2));
